...
struct MetadataDBT : public ::flatbuffers::NativeTable {
  ...
  MetadataDBT() = default;
  MetadataDBT(const MetadataDBT &o);
  MetadataDBT(MetadataDBT&&) FLATBUFFERS_NOEXCEPT = default;
  MetadataDBT &operator=(MetadataDBT o) FLATBUFFERS_NOEXCEPT;
};
...
struct MetadataDB FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  bool KeyCompareLessThan(const MetadataDB * const o) const {
...
inline MetadataDBT::MetadataDBT(const MetadataDBT &o)
      : address(o.address),
        asterisk((o.asterisk) ? new comm::datalayer::MetadataDBT(*o.asterisk) : nullptr),
        metadata((o.metadata) ? new comm::datalayer::MetadataT(*o.metadata) : nullptr) {
  childs.reserve(o.childs.size());
  for (const auto &childs_ : o.childs) { childs.emplace_back((childs_) ? new comm::datalayer::MetadataDBT(*childs_) : nullptr); }
}

inline MetadataDBT &MetadataDBT::operator=(MetadataDBT o) FLATBUFFERS_NOEXCEPT {
  std::swap(address, o.address);
  std::swap(childs, o.childs);
  std::swap(asterisk, o.asterisk);
...
inline void MetadataDB::UnPackTo(MetadataDBT *_o, const ::flatbuffers::resolver_function_t *_resolver) const {
  ...
  { auto _e = childs(); if (_e) { _o->childs.resize(_e->size()); for (::flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { if(_o->childs[_i]) { _e->Get(_i)->UnPackTo(_o->childs[_i].get(), _resolver); } else { _o->childs[_i] = std::unique_ptr<comm::datalayer::MetadataDBT>(_e->Get(_i)->UnPack(_resolver)); }; } } else { _o->childs.resize(0); } }
  { auto _e = asterisk(); if (_e) { if(_o->asterisk) { _e->UnPackTo(_o->asterisk.get(), _resolver); } else { _o->asterisk = std::unique_ptr<comm::datalayer::MetadataDBT>(_e->UnPack(_resolver)); } } else if (_o->asterisk) { _o->asterisk.reset(); } }
...
inline comm::datalayer::MetadataDB *GetMutableSizePrefixedMetadataDB(void *buf) {
  return ::flatbuffers::GetMutableSizePrefixedRoot<comm::datalayer::MetadataDB>(buf);
}
...
inline bool SizePrefixedMetadataDBBufferHasIdentifier(const void *buf) {
  return ::flatbuffers::BufferHasIdentifier(
      buf, MetadataDBIdentifier(), true);
}
