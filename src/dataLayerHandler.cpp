...
int main_datalayer()
{
  ...
  std::cout << "The datalayer main has been entered" << std::endl;

  auto mode_id_address = "ctrlx_nanotec/mode_id";
  ...
  // PROVIDER
  std::cout << "INFO Register 'ctrlx_nanotec' as root element with the nodes 'myInt64'" << std::endl;
  ...
  mode_id.setValue((int64_t)1);
  std::cout << "INFO Register node " << mode_id_address  << std::endl;
  comm::datalayer::DlResult result = provider->registerNode(mode_id_address, new MyProviderNode(mode_id));
  if (STATUS_FAILED(result))
  {
    std::cout << "WARN Register node " << mode_id_address  << " failed with: " << result.toString() << std::endl;
  }

  // CLIENT
  auto connectionString = getConnectionString(); // default: ctrlX CORE or ctrlX COREvirtual with Network Adpater
  std::cout << "INFO Creating ctrlX Data Layer client connection to " << connectionString << " ..." << std::endl; // it will say ipc://
  ...
  // Infintiy loop
    ...
    std::cout << "INFO Reading " << mode_id_address << " synchronously..." << std::endl;
    auto result = dataLayerClient->readSync(mode_id_address, &mode_id);
    
    if (result != DL_OK){
      std::cout <<"WARN Reading " << mode_id_address << " failed with: " << result.toString() << std::endl;
    } 
    else
    {
      if (mode_id.getType() == comm::datalayer::VariantType::INT64)
      {
        std::cout << "INFO Value of " << mode_id_address << ": " << double(mode_id) << std::endl;
      }
      else
      {
        std::cout << "WARN Value of " << mode_id_address << " has unexpected type: " << mode_id.typeAsString() << std::endl;
      }
    }
  ...
  provider->unregisterNode("ctrlx_nanotec/mode_id");
